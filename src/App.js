...
function App() {
  ...
  const [alertClassName, setAlertClassName] = useState("d-none");    // 'd-none' means 'display:none' so the alert is hidden by deault.
  ...
  const toggleRefresh = useCallback((status) => {
    ...
    if (status) {
      ...
      }, 600000);    // Runs every 1000ms.
      setTickInterval(i);
      console.log("setting tick interval to", i);    // 'i' is an ID value returned by setInterval() that can be used to stop the interval by calling clearInterval(i).
      console.log(tickInterval);
    } else {
      console.log("turning off ticking");
      console.log("turning off tick interval", tickInterval);
      setTickInterval(null);
      clearInterval(tickInterval);    // In-built React function
      console.log(tickInterval);
    }
  }, [tickInterval]);
